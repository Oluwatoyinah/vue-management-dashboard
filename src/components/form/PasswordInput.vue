...
<template>
  <div>
    <label class="text-[13px]" :htmlFor="{ name }">
      {{ label }}
      <span v-if="required" class="text-[#f00]">*</span>
    </label>
    <div class="relative">
      <input
        class="border border-[#33333330] block w-full py-3 px-3 rounded-[5px] outline-none mt-1"
        type="password"
        :name="{ name }"
        :id="{ name }"
      />
      <span class="absolute top-[25%] right-[10px]">
        <svg
          class="!w-[20px]"
          fill="none"
          height="24"
